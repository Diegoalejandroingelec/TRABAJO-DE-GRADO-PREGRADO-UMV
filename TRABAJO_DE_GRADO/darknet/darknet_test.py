...
def performDetect(imagePath, thresh= 0.20, configPath = "./cfg/yolov4-fallas-UMV.cfg", weightPath = "./backup/yolov4-fallas-UMV_50000.weights", metaPath= "./cfg/fallas-UMV.data", showImage= True, makeImageOnly = False, initOnly= False):
    """
    ...
    if showImage:
        try:
            ...
            for detection in detections:
                ...
                draw.set_color(image, (rr, cc), boxColor, alpha= 1)
                draw.set_color(image, (rr2, cc2), boxColor, alpha= 1)
                draw.set_color(image, (rr3, cc3), boxColor, alpha= 1)
                draw.set_color(image, (rr4, cc4), boxColor, alpha= 1)
                draw.set_color(image, (rr5, cc5), boxColor, alpha= 1)
...
def obtiene_path_img_ventaneadas(nombre_de_la_imagen,path_guarda_imagenes_ventaneadas):
    name_img=nombre_de_la_imagen.split("/")[-1][0:len(nombre_de_la_imagen.split("/")[-1])-4]
    lista_aux=glob.glob(path_guarda_imagenes_ventaneadas+'/*.jpg')
    lista_aux.sort()
    ventanas=[]
    cont=0
    while(path_guarda_imagenes_ventaneadas+'/'+name_img+'_v'+str(cont)+'.jpg' in lista_aux):
        ventanas.append(path_guarda_imagenes_ventaneadas+'/'+name_img+'_v'+str(cont)+'.jpg')
        cont=cont+1
...
if __name__ == "__main__":
    ...
    path_imagenes='/home/diego/TRABAJO-DE-GRADO-PREGRADO-UMV/TRABAJO_DE_GRADO/Performance_parameters/imagenes_test_UMV'
    ...
    for nombre_de_la_imagen in imagenes_de_prueba:
        detection_images.append(performDetect(imagePath=nombre_de_la_imagen,showImage=False))
        i=i+1
        if i%100==0:
            print(nombre_de_la_imagen)
        
    tmstmp2 = time.time()
    print('Total time elapsed = ', tmstmp2 - tmstmp1)
    save_obj(detection_images, 'informacion_imagenes_test_UMV' )
