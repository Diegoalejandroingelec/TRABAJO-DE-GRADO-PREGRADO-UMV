...
def csv_to_yolo_format(VoTT_csv,image_path,Folder_labels,path_clases_file,Folder_para_direccion_de_imagenes_train,Folder_para_direccion_de_imagenes_test,porcentaje_de_cada_clase):
    ...
    imagenes_con_clases_de_interes=list(set(multi_df['image']))
    ...
    num_folds=4
    imagenes_que_ya_fueron_de_test=[]
    img_sin_cla_ya_fue_de_test=[]
    for k in range(num_folds):
        ...
        imagenes_to_train=imagenes_que_ya_fueron_de_test.copy()
        for img_j in imagenes_to_train:
            aux_img=multi_df[multi_df['image'] == img_j]
            for index, row in aux_img.iterrows():
                codigo_clase=(row[["code"]].tolist())
                nombre_de_la_imagen=(row[["image"]].tolist())
        ...
        for index, row in multi_df.iterrows():  
            codigo_clase=(row[["code"]].tolist())
            nombre_de_la_imagen=(row[["image"]].tolist())
            if not (nombre_de_la_imagen[0] in imagenes_to_train):
                if nombre_de_la_imagen != nombre_de_la_imagen_anterior:
        ...
        for fnames in images_text:
            file = open(Folder_para_direccion_de_imagenes_train+'train'+str(k)+'.txt','a')
        ...
        imagenes_to_test=[]
        for x in list(set(multi_df['image'])):
            imagenes_to_test_disponible=True
            if x in imagenes_to_train:
                imagenes_to_test_disponible=False
            if imagenes_to_test_disponible:
                imagenes_to_test.append(x)
    
        imagenes_que_ya_fueron_de_test=imagenes_que_ya_fueron_de_test+imagenes_to_test.copy()   
        ...
        for fnames in images_text_1:
            file = open(Folder_para_direccion_de_imagenes_test+'test'+str(k)+'.txt','a')
            file.write(fnames+os.linesep )
            file.close()
            
        #print('images_to_train'+ str(len(imagenes_to_train)))
        #print('images_to_test'+ str(len(imagenes_to_test)))
        ...
        con_test=0
        imagenes_sin_clases_de_interes_to_train=img_sin_cla_ya_fue_de_test.copy()
        con_train=len(imagenes_sin_clases_de_interes_to_train)
        
        imagenes_sin_clases_de_interes_to_test=[]
        for n in imagenes_sin_clases_de_interes:
            if not((image_path+n) in imagenes_sin_clases_de_interes_to_train):
                if con_train<num_clases_de_interes_para_train:
                    imagenes_sin_clases_de_interes_to_train.append(image_path+n)
                    con_train+=1
                    
            if con_train>=num_clases_de_interes_para_train and not((image_path+n) in imagenes_sin_clases_de_interes_to_train):
                imagenes_sin_clases_de_interes_to_test.append(image_path+n)
                con_test+=1 
                
        img_sin_cla_ya_fue_de_test=img_sin_cla_ya_fue_de_test+imagenes_sin_clases_de_interes_to_test.copy()
        ...
        for n in imagenes_sin_clases_de_interes_to_train:
            ...
            file.write(n+os.linesep )
            file.close()
        #print('images_to_train_sin_clase'+ str(len(imagenes_sin_clases_de_interes_to_train)))
        #print('images_to_test_sin_clase'+ str(len(imagenes_sin_clases_de_interes_to_test)))
        ...
        print('')
        print('') 
        print('LAS CARACTERISTICAS DEL FOLD NUMERO '+str(k)+ ' SON: ') 
...
Folder_para_direccion_de_imagenes_train='/home/diego/TRABAJO-DE-GRADO-PREGRADO-UMV/TRABAJO_DE_GRADO/Annotation_images/'
Folder_para_direccion_de_imagenes_test='/home/diego/TRABAJO-DE-GRADO-PREGRADO-UMV/TRABAJO_DE_GRADO/Annotation_images/'

#PATH DE LA UBICACIÓN DEL ARCHIVO CSV DE LAS ETIQUETAS Y PATH DE LAS IMÁGENES 
VoTT_csv = '/home/diego/TRABAJO-DE-GRADO-PREGRADO-UMV/TRABAJO_DE_GRADO/Annotation_images/Info_dataset/JPEGImages/Dia_1-export.csv'
